...
- Dashboard HTML also embeds that payload in `#scanCatalogBootstrap` so Scan studio paints all sleeves after a refresh even if the catalog GET is cached or delayed. `scanStudio.js` is cache-busted via an import map (`?v=` app version).
- `POST /api/scan` body extras: `universe_preset`, `scan_timeframe`, `strategy_ids`.

Dashboard: `#scanStudioPanel` on the Today scan lane is the **live local operator control** (and the same static panel on SaaS). All 20 sleeves are listed at once, grouped by horizon; timeframe tabs set `scan_timeframe` for the next Run scan and do not hide rows. Prefs persist in `tradingbot.scan.studio.v2`. The v1 key `tradingbot.scan.studio` is read once: universe and custom tickers are kept, but `strategy_ids` reset to that tab's `default_strategy_id` so leftover "select all" sessions do not OR every sleeve. Deep link: `?section=scanstudio`. `GET /api/public-config` advertises `scan_studio.enabled` on both local and SaaS. Editorial visual language follows the Old Logan Figma DS (`ol/bg`, `ol/surface`, `ol/accent`, `ol/gold`). Live execution copy stays daily Stage 2 / VCP — extra sleeves remain shadow/research.
