...
# Promotion-gate helpers live next to this script. Add scripts/ to path so
# we can import without packaging.
_SCRIPTS_DIR = Path(__file__).resolve().parent
if str(_SCRIPTS_DIR) not in sys.path:
    sys.path.insert(0, str(_SCRIPTS_DIR))


from _strategy_gates import DEFAULT_LOW_CONFIDENCE_N, annotate_bucket_confidence
...
def main() -> None:
    ...
    # Sample-size discipline: tag low-N buckets so downstream consumers
    # (dashboards, auto-tuners) know they're noisy and must not drive
    # hard policy decisions on their own.
    annotate_bucket_confidence(score_stats, n_field="count", min_n=DEFAULT_LOW_CONFIDENCE_N)

    annotate_bucket_confidence(vcp_stats, n_field="count", min_n=DEFAULT_LOW_CONFIDENCE_N)
    ...
    output = {
        ...
        "confidence_policy": {
            ...
            "applies_to": [
                ...
                "top_decile_signal_score_stats",

                "top_decile_vcp_ratio_stats",
