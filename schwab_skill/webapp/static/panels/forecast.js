...
export function buildForecastSummary(data) {
  ...
  const mode = escapeHtml(safeText(data.scanner_mode || "off"));
  return `
    <div class="forecast-summary">
      ...
      <div class="forecast-metric">
        <span class="forecast-metric-label">Expected move (${horizon}d)</span>
        <strong>${retText}</strong>
      </div>
      <div class="forecast-metric">
        <span class="forecast-metric-label">Confidence</span>
        <strong>${conf} (${confPct})</strong>
      </div>
      <p class="forecast-foot muted">
        Model ${model} · scanner mode ${mode} · advisory only, not a trade signal.
      </p>
    </div>
  `;
}
